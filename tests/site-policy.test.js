...
test("확장 새로고침으로 runtime이 무효화되면 이전 content 인스턴스는 오류 없이 리스너를 해제한다", () => {
  const documentListeners = new Map();
  let runtimeListener = null;
  let runtimeListenerRemoved = false;
  let shortcutStops = 0;
  let sendMessageCalls = 0;
  const document = {
    location: { hostname: "laftel.net" },
    documentElement: { setAttribute() {} },
  ...
  const context = {
      ...
      runtime: {
        onMessage: {
          ...
          removeListener(listener) {
            runtimeListenerRemoved = listener === runtimeListener;
          }
        },
        sendMessage() {
          sendMessageCalls += 1;
    ...
    ViewTune: {
      BUILD_ID: "test-build",
      pageCapability: () => ({ supported: true }),
      runtimeIdentity: () => ({ buildId: "test-build" }),
      unsupportedPageResult: () => null,
      VideoController: class {
        videoFromElement() { return true; }
        isUsableVideo() { return true; }
      },
      ShortcutController: class {
        start() {}
        stop() { shortcutStops += 1; }
      }
    }
  };

  vm.runInNewContext(mainSource, context, { filename: "main.js" });
  const stalePointerListener = documentListeners.get("pointerdown");

  assert.doesNotThrow(() => stalePointerListener({ target: {} }));
  assert.equal(sendMessageCalls, 1);
  assert.equal(shortcutStops, 1);
  assert.equal(runtimeListenerRemoved, true);
  assert.equal(documentListeners.has("pointerdown"), false);
  assert.equal(documentListeners.has("play"), false);
